...
class WebhooksApi
{
    ...
    public function listWebhooksWithHttpInfo(string $contentType = self::contentTypes['listWebhooks'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\OpenAPI\Client\Model\WebhookPaginatedList' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\OpenAPI\Client\Model\WebhookPaginatedList' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\OpenAPI\Client\Model\WebhookPaginatedList', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
